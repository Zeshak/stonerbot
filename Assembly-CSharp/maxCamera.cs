...
public class maxCamera : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        {
            this.desiredDistance -= (((Input.GetAxis("Mouse Y") * UnityEngine.Time.deltaTime) * this.zoomRate) * 0.125f) * Mathf.Abs(this.desiredDistance);
        }
        else if (Input.GetMouseButton(0))
        {
            ...
            this.currentRotation = base.transform.rotation;
            this.rotation = Quaternion.Lerp(this.currentRotation, this.desiredRotation, UnityEngine.Time.deltaTime * this.zoomDampening);
            base.transform.rotation = this.rotation;
        }
        else if (Input.GetMouseButton(2))
        {
            this.target.rotation = base.transform.rotation;
            this.target.Translate((Vector3) ((Vector3.right * -Input.GetAxis("Mouse X")) * this.panSpeed));
            this.target.Translate((Vector3) ((base.transform.up * -Input.GetAxis("Mouse Y")) * this.panSpeed), Space.World);
        }
        this.desiredDistance -= ((Input.GetAxis("Mouse ScrollWheel") * UnityEngine.Time.deltaTime) * this.zoomRate) * Mathf.Abs(this.desiredDistance);
        this.desiredDistance = Mathf.Clamp(this.desiredDistance, this.minDistance, this.maxDistance);
        this.currentDistance = Mathf.Lerp(this.currentDistance, this.desiredDistance, UnityEngine.Time.deltaTime * this.zoomDampening);
